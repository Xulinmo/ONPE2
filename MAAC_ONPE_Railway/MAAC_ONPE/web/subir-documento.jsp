                ...
                <div class="card border-0 shadow-sm mx-auto" style="max-width: 600px;">
                    ...
                    <div class="card-body p-4">
                        <form action="SubirArchivoExtraServlet" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold">Seleccionar Archivos (Múltiples PDFs) <span class="text-danger">*</span></label>
                                <input type="file" name="archivos[]" class="form-control" accept="application/pdf, .pdf" multiple required>
                                <small class="text-muted">Puede seleccionar varios archivos a la vez manteniendo presionada la tecla Ctrl.</small>
                            </div>
        ...
        <script>
            ...
            if (msg === 'ok_masivo') {
                Swal.fire({
                    ...
                    text: 'Todos los documentos seleccionados fueron adjuntados al expediente del postulante correctamente.',
                    confirmButtonColor: '#28a745'
                }).then(() => {
                    // Limpia la URL para estética y evitar que repita al recargar
            ...
            if (error === 'solo_pdf') {
                Swal.fire({
                    icon: 'error',
                    title: 'Formato Rechazado',
                    text: 'El sistema de la ONPE solo permite la subida de archivos en formato PDF.',
            ...
            if (error === 'dni') {
                Swal.fire({
                    ...
                    text: 'El DNI ingresado no corresponde a ningún postulante en el sistema.',
                    confirmButtonColor: '#d33'
                }).then(() => {
                    window.history.replaceState(null, null, window.location.pathname);
                });
            ...
            if (error === 'bd') {
                Swal.fire({
                    icon: 'error',
                    title: 'Error Interno',
                    text: 'Hubo un problema al conectar con el servidor. Inténtelo más tarde.',
                    confirmButtonColor: '#d33'
                }).then(() => {
                    window.history.replaceState(null, null, window.location.pathname);
                });
