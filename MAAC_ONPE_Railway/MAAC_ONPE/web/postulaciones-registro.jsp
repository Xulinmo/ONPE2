...
<html lang="es">
    ...
    <body class="bg-light">
        ...
        <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
        <script>
            // Leemos la URL completa actual
            const urlActual = window.location.href;

            // Si la URL contiene la palabra "solo_pdf" en cualquier parte, dispara la alerta
            if (urlActual.includes('solo_pdf')) {
                Swal.fire({
                    icon: 'error',
                    title: 'Formato Rechazado',
                    text: 'Verifica que el archivo sea realmente un PDF (que termine en .pdf).',
                    confirmButtonColor: '#d33'
                });
            }

            // Si la URL contiene un error de Base de Datos
            if (urlActual.includes('error=1')) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error Interno',
                    text: 'Hubo un problema al guardar en la base de datos. Verifica los campos.',
                    confirmButtonColor: '#d33'
                });
            }

            // VALIDACIÓN DE DNI DUPLICADO (RF-23)
            if (urlParams.get('error') === 'dni_duplicado') {
                Swal.fire({
                    icon: 'error',
                    title: 'DNI Duplicado',
                    text: 'El número de documento ingresado ya se encuentra registrado en el sistema. No se permiten expedientes duplicados.',
                    confirmButtonColor: '#d33'
                });
            }
        </script>
